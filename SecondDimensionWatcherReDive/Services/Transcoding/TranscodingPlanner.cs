...
internal static class TranscodingPlanner
{
        ...
        bool burnBitmapSubtitles)
    {
        ...
        var copyVideo = video.CodecName.Equals("h264", StringComparison.OrdinalIgnoreCase)
                        && selection.Quality == "auto"
                        && bitmapToBurn is null;
        var copyAudio = audio is null || HlsAudioCodecs.Contains(audio.CodecName);
        var direct = IsDirectPlayContainer(extension, video.CodecName, audio?.CodecName)
                     && selection.Quality == "auto"
                     && bitmapToBurn is null;
        var strategy = direct
    ...
    private static bool IsDirectPlayContainer(string extension, string videoCodec, string? audioCodec)
    {
        if (extension.Equals(".mp4", StringComparison.OrdinalIgnoreCase)
            || extension.Equals(".m4v", StringComparison.OrdinalIgnoreCase))
            return videoCodec.Equals("h264", StringComparison.OrdinalIgnoreCase)
        ...
        var supportedVideo = videoCodec.Equals("vp8", StringComparison.OrdinalIgnoreCase)
                             || videoCodec.Equals("vp9", StringComparison.OrdinalIgnoreCase)
                             || videoCodec.Equals("av1", StringComparison.OrdinalIgnoreCase);
