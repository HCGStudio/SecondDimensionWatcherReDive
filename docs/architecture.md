...
- `MediaTimelineController` (`/api/playback/timeline`) — Shared media-version-bound OP/ED ranges and chapters, with opt-in season defaults scoped to title/season/group. Resolving a timeline requires physical length and modification time; missing metadata or media yields 404. In-player auto-skip is opt-in per profile; progress sampling checks the active ending range before either manual or automatic skipping can enqueue a watched update. Timeline reads, saves, season acceptance and deletion re-read the resolved mapping under the shared mapping transaction lock and validate its owner, virtual/physical path, store, current season/group identity and physical version before accessing timelines. Missing mappings return 404 and changed mutation targets return 409, including on first saves. Episode timelines belong to a mapping and cascade on its deletion; a mapping replacement clears that mapping's episode timelines and season acceptances. Reading or writing a new physical version retires only the same mapping's obsolete episode timelines and acceptances, retaining the current version and shared season templates. The ownership migration recovers legacy episode mapping IDs from matching season acceptances. Legacy episodes without a recoverable owner are preserved to avoid losing potentially valid user data; reading their exact verified current version claims them for future cleanup. Logical playback exports/imports retain the AutoSkip preference; false is omitted from JSON to preserve checksums of older format-1 exports.
