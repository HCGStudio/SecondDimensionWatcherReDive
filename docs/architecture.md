...
- **FetchRemoteTorrentBackgroundService** — Recovers persisted tracked downloads, polls bounded hash batches at adaptive intervals and persists remote completion.
- **UpdateDownloadStatusBackgroundService** — Caches the bounded progress stream.
- **CompleteDownloadBackgroundService** — Claims durable jobs with independent worker scopes and advances mapping, notification and plugin stages; Channel messages only wake the workers sooner.
...
- `src/i18n/resources.ts` — statically imports the active feature locale JSON files and assembles the resources map. The `settings` namespace is currently defined inline here for all three languages, with only `system.reauthenticatePrompt`; the existing `locales/*/settings.json` files are not loaded.
- `src/i18n/locales/{zh-CN,en,ja}/*.json` — feature/page namespaces including library, metadata review, accounts, incidents, todos and watchlist as well as the original player/feed/file resources. To add a string to a loaded namespace, edit all three language files; edit the inline entries in `resources.ts` for the current Settings prompt. To add a language, add matching JSON files, import and register them in `resources.ts`, and add it to `supportedLanguages` in `src/i18n/index.ts`.
