...
`LogicalDataTransferController` and `LogicalDataTransferRepository` export format 2 and accept formats 1 and 2. `recognition-rules` is a separate category included in `all`, with its own `LogicalRecognitionRule` payload and a 200-rule limit. Format-1 checksum canonicalization retains the original bundle shape and category enum, including the old meaning and spelling of `All`; the optional new bundle field is omitted for legacy input. Rule IDs are portable conflict keys, source feeds resolve by URL, and missing source dependencies are reported as skipped without widening scope. Enabled imported rules use the same strict remote TMDB series/season lookup as rule validation, before opening the import transaction or taking locks; unavailable lookups return 503 and missing targets/seasons return 422. Disabled rules may be imported offline. Ordinary rule creation checks the shared 200-rule limit and inserts atomically under the same mapping/rule lock order as logical imports. Import shares the mapping/rule transaction locks with inference, resets changed or new rules’ `EffectiveFrom` to the local import time, increments local revisions on overwrite and clears source-instance correction references. It never reprocesses historical releases automatically. See [backup and restore](backup-restore.md) for category selection and conflict handling.
