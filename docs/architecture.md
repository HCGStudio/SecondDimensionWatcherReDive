...
React + TypeScript with Tailwind CSS for styling and Radix UI for accessible interactive primitives (Dialog, Toast, Progress). Uses SWR for data fetching, React Router for routing, lucide-react for icons, Artplayer for video playback, react-i18next for localization (zh-CN / en / ja). The workbench UI uses a white canvas, mint surfaces, forest-green accents, sans-serif typography, and compact media lists. Shared tokens are defined in `tailwind.config.js`, base styles in `src/styles.css`, and the SVG brand mark in `src/favicon.svg` (also reused by `BrandIcon`). Design exploration HTML is not part of the application or repository deliverable.

**Pages:**
- Main (`/`, with `/main` alias) — Workbench with resumable playback, compact anime rows grouped by TMDB ID, catalog and uncategorized cursor pagination, and a real download/subscription overview. Additional unfinished videos remain available in an expandable strip; overview pause/resume follows content-write permissions and links to the full download and feed pages.
...
- Search (`/search`) — URL-synced library filters, completeness plans, and version upgrades
- Todos (`/todo`) and incidents (`/incidents`) — Administrator queues with filters, batch actions, and deep links
- Metadata review (`/metadata-review`) — Administrator preview, corrections, and undo
...
- `src/i18n/locales/{zh-CN,en,ja}/*.json` — 16 namespaces grouped by feature/page surface, including accounts, library, metadata review, settings, incidents, and todos. `common.appTitle` supplies the localized browser title independently of the navigation brand label. To add a string, edit all three language files. To add a language, drop a folder of JSON files matching the structure and add it to `supportedLanguages` in `src/i18n/index.ts`.
