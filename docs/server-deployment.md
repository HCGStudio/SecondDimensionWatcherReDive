...
非 MKV 媒体发生浏览器解码或格式不支持错误时，也会进入服务端后备；该请求强制生成 HLS，并与原文件直放使用不同的缓存标识，避免服务端再次返回已失败的直放路径。Linux 本地存储通过仍保持打开的文件描述符向 FFmpeg 提供可定位输入，支持索引位于文件尾部的 MP4/MOV，且不按已验证文件的名称重新打开。其他平台或不提供本地文件句柄的存储 Provider 继续使用管道输入；需要随机读取的非 faststart MP4 在该模式下仍可能无法转换，可先将容器索引移到文件开头。输入仅允许受支持的媒体容器以及 file/pipe 协议，不允许嵌套播放列表或远程 URL。

原文件播放链接响应提供到期时间。播放器在有效期约 80% 时重新签发视频及外挂字幕链接，并保留当前位置、播放/暂停、速度、音量和静音状态；后台标签页恢复或过期后的播放错误也会触发重取。续签需要当前登录会话，切换身份会取消旧请求，不延长已撤销会话或旧链接的权限。
