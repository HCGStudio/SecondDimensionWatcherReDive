...
`sdw-backup` 提供可自动化的本地备份目标；系统包和应用容器均携带该命令。备份由 PostgreSQL custom-format dump、部署配置、旧版 `password.json`（存在时）、Data Protection 密钥环和插件 manifest 组成。家庭账户、密码哈希、配置档案和会话随数据库一起备份；没有旧版密码文件的安装会保存空的兼容文件。配置迁移保留了 `Authentication:BootstrapCredentialsFile` 引用时，必须用 `--password-file` 备份该文件；恢复到引用路径，或同步更新引用后再启动。下载媒体、导入的原始媒体、qBittorrent 数据与 Valkey 缓存不在其中。
...
追番、媒体时间段、识别规则命中历史、多来源决策和容量队列需要通过完整 PostgreSQL 备份恢复，当前逻辑 JSON 类别不导出这些数据；长期识别规则及多来源订阅配置本身可分别通过 `recognition-rules` 和 `multi-source-subscriptions` 类别迁移。升级后应重新创建完整备份；恢复容量队列还需与独立恢复的 qBittorrent 状态及实际卷对账。
